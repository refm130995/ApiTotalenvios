...
export class BranchOfficesService {
  constructor(
    @InjectModel('BranchOffices')
    public readonly branchOfficesModel: Model<BranchOffices>,
  ) {}

  async insert(branch: CreateBranchDto) {
    if (
      await this.branchOfficesModel.exists({
        email: branch.email,
      })
    ) {
  ...
  async Updateservices(services: any) {
    let result = await this.branchOfficesModel.findByIdAndUpdate(
      services._id,
      services,
    );
    if (result) {
