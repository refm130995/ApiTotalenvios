...
export class QuotesService {
    constructor(
        @InjectModel('Queou') public readonly QueouModel: Model < Queou >) {}
    
    async inQueue(queue: CreateQueueDTO){
        if (await this.QueouModel.exists({
             userId: queue.userId,
        ...
        let result = await new this.QueouModel(queue).save();
            var diff = moment(queue.date, "YYYY-MM-DD HH:mm:ss").diff(Date.now(), 'ms'); // Diff in days
            setTimeout(()=>{ 
                //Mandar correo
