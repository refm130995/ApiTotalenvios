...
import { Country, Cities, CountryDTO, FindCityDTO, CityDto } from '../schemas/country.schemas';
...
export class CountryService {
    ...
    async Update(country: any) {
        let result = await this.countryModel.findByIdAndUpdate(country._id, country)
        if (result) {
            return result;
        } else {
    ...
    async Add(country: CountryDTO) {
        console.log(country);
        if (await this.countryModel.exists({
                name: country.name
            })) {
            throw new HttpException({
                status: HttpStatus.EXIST,
    ...
    async AddCities(data: CityDto) {
        console.log(data);
        if (await this.citiesModel.exists({
            city: data.city
        })) {
        throw new HttpException({
            status: HttpStatus.EXIST,
